...
function buildSVG(calendar) {
  ...
  const svgElements = [];
  const STROKE_COLOR = 'rgba(0,0,0,0.2)'; 
  ...
  weeks.forEach((week, wIndex) => {
    week.contributionDays.forEach((day, dIndex) => {
      ...
      svgElements.push(`<g id="d-${day.date}"><polygon points="${leftPts}" fill="${COLOR_LEFT[tier]}" stroke="${STROKE_COLOR}" stroke-width="0.2"/><polygon points="${rightPts}" fill="${COLOR_RIGHT[tier]}" stroke="${STROKE_COLOR}" stroke-width="0.2"/><polygon points="${topPts}" fill="${COLOR_LEVELS[tier]}" stroke="${STROKE_COLOR}" stroke-width="0.2"/></g>`);
    });
  });
